...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
       ...
       <link rel="icon" href="../img/confirm.png" type="image/x-icon">
...
<body>
    ...
    <div class="div1">
        ...
        <?php
                ...
                $sql="select mshh,soluong,giadathang,trangthai,giamgia from chitietdathang where sodondh='$sodondh'";   
                $result=mysqli_query($conn, $sql);
                $product=mysqli_fetch_assoc($result);
                $mshh=$product['mshh'];
                $soluong=$product['soluong'];
                $temp=$product['trangthai'];
                $giamgia=$product['giamgia'];
                $giadathang=$product['giadathang'];
            
                if($temp==2){
                    $trangthai="Đơn Hàng Được Xác Nhận ";
                }
                else if($temp==-1){
                    $trangthai='Đơn Hàng Đã Bị Hủy';
                }
                else $trangthai='Đã Đặt Hàng';
                $tongtien=number_format($giadathang-$giamgia,0,',','.');
                $sql="select hinhanh,tenhh from hanghoa where mshh='$mshh'";   
                $result=mysqli_query($conn, $sql); 
                $product=mysqli_fetch_assoc($result);
            ...
            <div class='supchitiet'>
                ...
                <div class='package'>
                    ...
                    <div class='package_right'>
                        <p> Tên Sản Phẩm: ".$tenhh."</p>
                        <p> Số Lượng: ".$soluong."</p>
                        <p>Tổng Tiền: ".$tongtien." đ</p>
                        <p>Trạng Thái: <span>".$trangthai."</span></p>
                       
                        <p>Giao hàng: </p>
                        <p>Cảm ơn bạn đã đặt hàng của shop. Đơn hàng được xác nhận sẽ vận chuyển trong vòng 3 ngày kể từ khi đặt hàng</p>
